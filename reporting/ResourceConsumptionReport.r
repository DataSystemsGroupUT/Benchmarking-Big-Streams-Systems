...
generateStreamServerLoadReport <- function(engines, tps, duration){
   ...
   for(i in 1:15) {
     ...
     charts[2*i] <- ggplot(data=cpuUsage, aes(x=TIME, y=USAGE, group=ENGINE, colour=ENGINE)) + 
     ...
     charts[2*i+1] <- ggplot(data=memoryUsage, aes(x=TIME, y=USAGE, group=ENGINE, colour=ENGINE)) + 
       geom_smooth(method="loess", se=F) + 
       scale_y_continuous(breaks= pretty_breaks()) +
       guides(fill=FALSE) +
